        ...
        if isinstance(goal,ThingClass):
            ...
            if isinstance(self.goal.Class,Or):  
                ...
                if self.children[0].goal.type == owlready2.SOME:
                    self.eq = NodeGoal((self.children[0].goal.property).only(Not(self.children[0].goal.value))) # diventa only
                elif self.children[0].goal.type == owlready2.ONLY:
                    self.eq = NodeGoal((self.children[0].goal.property).some(Not(self.children[0].goal.value))) # diventa some
            else:
...
class BaseGoalBuilder(GoalBuilder):
    ...
    def buildGoals(self):
        ...
        res = self.parser.parse(goal_line)
        print(res,goal_line)
